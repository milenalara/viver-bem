...
void cadastraPaciente(FILE *fPacientes)
{
    ...
    do
    {
        ...
        if ((pac.telefone>=99999999999) || (pac.telefone<=1000000000 && pac.telefone>0) || controleTelefone!=0)
            printf ("\nErro! Formato telefone incorreto\nDigite novamente\n");
        printf("Informe o telefone(Formato: (00)00000-0000):\n");
        fflush(stdin);
        scanf("%llu", &pac.telefone);
        controleTelefone++;
    }
    while ((pac.telefone>=99999999999) || (pac.telefone<=1000000000));
...
void cadastraMedico(FILE *fMedicos)
{
    ...
    do
    {
        ...
        if ((med.telefone>=99999999999) || (med.telefone<=1000000000 && med.telefone>0) || controleTelefone!=0)
            printf ("\nErro! Formato telefone incorreto\nDigite novamente\n");
        printf("Informe o telefone(Formato: (00)00000-0000):\n");
        fflush(stdin);
        scanf("%llu", &med.telefone);
        controleTelefone++;
    }
    while ((med.telefone>=99999999999) || (med.telefone<=1000000000));
    ...
    do
    {
        ...
        if (posicaoMed != -1)
        {
            ...
            if (consAux.data.ano == cons.data.ano && consAux.data.dia == cons.data.dia && consAux.data.mes == cons.data.mes)
            {
                ...
                {
                    printf("\nDiferença menor do que 30 minutos da consulta: %02d:%02d\n", consAux.data.hora, consAux.data.minuto);
                    controleHora = 1;
                }
                else
                    controleHora = 0;
...
void cancelaConsulta(FILE *fConsultas)
{
   ...
   rename("consultas_new.dat", "consultas.dat");
    fConsultas = fopen("consultas.dat", "w+b");
